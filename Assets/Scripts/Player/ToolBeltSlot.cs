...
public sealed class ToolBeltSlot
{
        ...
        UnityEngine.Object context)
    {
        ...
        {
            DestroyVisual(assignOwnerToolbelt);
            return;
        }

        if (visual != null && visual.Instance != null && visual.RegistryIndex == RegistryIndex)
        {
            UpdateVisual(definition, determineMountType, resolveMountTarget);
            return;
        }
    ...
    public GameObject DestroyVisual(Action<GameObject, bool> assignOwnerToolbelt)
    {
        if (visual == null)
            return null;
    ...
    private void RefreshTargetPose()
    {
        ...
        Vector3 worldPos = target ? target.TransformPoint(offsetPos) : root.TransformPoint(offsetPos);
        Quaternion worldRot = target ? target.rotation * offsetRot : root.rotation * offsetRot;
